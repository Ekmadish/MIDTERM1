...
calculation(String input) {
  ...
  if (num1List.length != 0 && num2List.length != 0 && input == "=") {
    ...
    allInputs = ["$result"];
    return;
  } else if (num2List.length == 0 && input == "=") return;
...
void allOperations() {
  if (_parameters[_parameters.length - 1] == "+") {
    ...
    double remain = remainder(num1List.join(), num2List.join());
    num1List = ["$remain"];
    num2List = [];
    result = remain;
  }
...
remainder(String num1, String num2) {
  double remainderResult = double.parse(num1) % double.parse(num2);
